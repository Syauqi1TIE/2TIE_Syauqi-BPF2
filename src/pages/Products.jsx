...
import { Link } from "react-router-dom"; // <--- PERBAIKAN 1: Wajib import Link dari react-router-dom
import PageHeader from "../components/PageHeader";

export default function Products() {
  const breadcrumb = ["Dashboard", "Product List"];
  
  // State untuk menyimpan data API dan error
  const [products, setProducts] = useState([]);
  const [error, setError] = useState(null);
  const [query, setQuery] = useState("");

  // Fitur pencarian dengan Debounce 500ms
  useEffect(() => {
  ...
  return (
    <div>
      ...
      {/* Menampilkan pesan error di atas tabel jika API gagal di-fetch */}
      {errorInfo}
      ...
      <input
        ...
        className="mb-4 p-3 w-full bg-white rounded-2xl shadow-lg border border-gray-100 focus:outline-emerald-500"
      />
        ...
        <tbody className="bg-white divide-y divide-gray-100 text-sm text-gray-800">
          {products.map((item, index) => (
            ...
            >
              ...
              {/* PERBAIKAN 2: Mengubah Nama menjadi Link Detail */}
              ...
              <td className="px-6 py-4">{item.brand || "Unknown Vendor"}</td>
            </tr>
          ))}
        </tbody>
      </table>
